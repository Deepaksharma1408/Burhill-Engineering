...
export const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <div
      className={`group relative flex flex-col justify-between p-6 sm:p-8 rounded-lg border transition-all duration-300 ${
        featured
          ? 'bg-navy-dark border-gold text-white shadow-xl hover:border-gold-light'
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className={`p-3 rounded border ${featured ? 'bg-navy border-gold text-gold' : 'bg-slate-bg border-slate-border text-navy group-hover:border-gold group-hover:text-gold'} transition-colors`}>
            <IconComponent className="w-6 h-6" />
          </div>
          <span className={`font-mono text-xs font-semibold px-2.5 py-1 rounded border ${featured ? 'border-navy-border bg-navy text-gold' : 'border-slate-border bg-slate-bg text-slate-muted'}`}>
            PILLAR #{service.id.slice(0, 8)}
          </span>
        </div>

        {/* Title */}
        <h3 className={`text-xl font-serif font-bold ${featured ? 'text-white' : 'text-navy group-hover:text-gold'} transition-colors`}>
          {service.title}
        </h3>
        ...
        {expandable && isExpanded && (
          <div className="pt-4 border-t border-slate-border/20 space-y-3 animate-fadeIn">
      ...
      <div className="pt-6 mt-6 border-t border-slate-border/30 flex items-center justify-between">
        ...
        <div className="flex space-x-1">
          {service.sectors.map((sec, i) => (
            <span key={i} className="text-[10px] font-mono px-2 py-0.5 rounded bg-navy-light/10 text-slate-muted">
              {sec}
            </span>
          ))}
        </div>
